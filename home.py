...
def showLink(usr):
    ...
    while len(likelist) > 0:
        n = maxLike(likelist)
        forms += "<tr>"
        forms += "<td><form action='pageform2.py' style='display:inline;' method='POST'>" + userdata
...
if 'usr' not in d or 'password' not in d or 'level' not in d:
    ...
    if level == '0':
        ...
        print "<br><center><b><form action='pageform2.py' method='POST'><input type='submit' name='make' class='linkbutton' value='Create Page'>"
        print userdata + "</form></b>"
        print """<b><form action='game.py' method='POST'>""" + userdata
        print """<input type='submit' name='game' value='The Word Game' class='linkbutton'></form>"""
        print """<form action='trials.py' method='POST'>""" + userdata + "<input name='qnumber' value='0' type='hidden'><input name='questionNumber' type='hidden' value='0'>"
        print """<input type='submit' name='trials' value='Raise Up Your Nerd Level' class='linkbutton'></form></b>"""
        print """<form action='FAQ.py' method='POST'>"""
	...
	if level == '5':
            ...
            print "<form action='admin.py' method='POST'>" + userdata + "<input type='text' required name='blockusr'><input type='submit' name='block' value='Block Users'></form>"
        print "</body></html>"
